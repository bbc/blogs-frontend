    ...
    caches.keys().then(function (names) {
        ...
        for (let name of names) {
            ...
            if (name != cacheKey) {
                ...
                if (name == cacheKey) {
                    console.log('That\'s because it\'s the current cache! We don\'t want to delete that!');
                }
            }
        }
...
// If it is, we return it from the cache, if it isn't, we return it from the network and then cache it.
    ...
    event.respondWith(
        caches.open(cacheKey).then((cache) => {
            return cache.match(event.request).then((response) => {
                ...
                return cache.match('/blogs/offline');
            });
        })
    );
});
